...
def encode(model, X, use_norm=True, verbose=True, batch_size=128, use_eos=False):
    ...
    for k in list(ds.keys()):
        ...
        for minibatch in range(numbatches):
            ...
            if use_eos:
                embedding = numpy.zeros(
                    (k+1, len(caps), model['options']['dim_word']), dtype='float32')
            else:
                embedding = numpy.zeros(
                    (k, len(caps), model['options']['dim_word']), dtype='float32')
            ...
            if use_eos:
                ff = model['f_w2v'](embedding, numpy.ones(
