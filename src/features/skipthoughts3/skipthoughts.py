...
#-----------------------------------------------------------------------------#
path_to_models = 'res/pretrained/skipthoughts/'
path_to_tables = 'res/pretrained/skipthoughts/'
#-----------------------------------------------------------------------------#
...
def load_tables():
    ...
    """
    words = []
    utable = numpy.load(path_to_tables + 'utable.npy', allow_pickle=True, fix_imports=True, encoding="latin1")
    btable = numpy.load(path_to_tables + 'btable.npy', allow_pickle=True, fix_imports=True, encoding="latin1")
...
def encode(model, X, use_norm=True, verbose=True, batch_size=128, use_eos=False):
    ...
    for k in list(ds.keys()):
        if verbose:
            print(k)
        # equivalent to ceiling of the division
        numbatches = -(-len(ds[k]) // batch_size)
