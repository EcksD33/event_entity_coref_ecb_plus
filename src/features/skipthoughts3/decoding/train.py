...
def trainer(X, C, stmodel,
    ...
    f_grad_norm = theano.function(
        inps, [(g**2).sum() for g in grads], profile=False)
    f_weight_norm = theano.function(
        [], [(t**2).sum() for k, t in tparams.items()], profile=False)
    ...
    train_iter = homogeneous_data.HomogeneousData(
        [X, C], batch_size=batch_size, maxlen=maxlen_w)
    ...
    for eidx in range(max_epochs):
        ...
        for x, c in train_iter:
            ...
            x, mask, ctx = homogeneous_data.prepare_data(
                x, c, worddict, stmodel, maxlen=maxlen_w, n_words=n_words)
            ...
            if numpy.mod(uidx, dispFreq) == 0:
                print('Epoch ', eidx, 'Update ',
                      uidx, 'Cost ', cost, 'UD ', ud)
