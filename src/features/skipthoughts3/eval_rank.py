...
def init_params(options):
    ...
    params = get_layer('ff')[0](
        options, params, prefix='ff_im', nin=options['dim_im'], nout=options['dim'])
    params = get_layer('ff')[0](
        options, params, prefix='ff_s', nin=options['dim_s'], nout=options['dim'])
...
def load_params(path, params):
    ...
    for kk, vv in params.items():
        if kk not in pp:
            raise Warning(f'{kk} is not in the archive')
        params[kk] = pp[kk]
    return params
...
def build_model(tparams, options):
    ...
    # image embedding
    lim = get_layer('ff')[1](tparams, im, options,
                             prefix='ff_im', activ='linear')
    lcim = get_layer('ff')[1](tparams, cim, options,
                              prefix='ff_im', activ='linear')

    # sentence embedding
    ls = get_layer('ff')[1](tparams, s, options, prefix='ff_s', activ='linear')
    lcs = get_layer('ff')[1](tparams, cs, options,
                             prefix='ff_s', activ='linear')
    ...
    cost_im = options['margin'] - \
        (lim * ls).sum(axis=1) + (lim * lcs).sum(axis=1)
    cost_im = cost_im * (cost_im > 0.)
    cost_im = cost_im.sum(0)

    cost_s = options['margin'] - \
        (ls * lim).sum(axis=1) + (ls * lcim).sum(axis=1)
...
def build_encoder(tparams, options):
    ...
    # embeddings
    eim = get_layer('ff')[1](tparams, im, options,
                             prefix='ff_im', activ='linear')
...
def adam(lr, tparams, grads, inp, cost):
    gshared = [theano.shared(p.get_value() * numpy.float32(0.),
                             name=f'{k}_grad') for k, p in tparams.items()]
    ...
    f_update = theano.function(
        [lr], [], updates=updates, on_unused_input='ignore')
...
def evaluate(X, saveto, evaluate=False, out=False):
    ...
    if evaluate:
        ...
        print(f"Image to text: {r1:.1f}, {r5:.1f}, {r10:.1f}, {medr:.1f}")
        (r1i, r5i, r10i, medri) = t2i(lim, ls)
        print(f"Text to image: {r1i:.1f}, {r5i:.1f}, {r10i:.1f}, {medri:.1f}")
    if out:
        return lim, ls
...
def trainer(train, dev,  # training and development tuples
    ...
    for eidx in range(max_epochs):
        ...
        for minibatch in range(numbatches):
            ...
            cinds_im = conprng_im.random_integers(
                low=0, high=len(train[0])-1, size=ncon * len(im))
            cinds_s = conprng_s.random_integers(
                low=0, high=len(train[0])-1, size=ncon * len(s))
